...
module Acquisition
  module RemoteOk
    class Collector
      ...
      EVIDENCE_KINDS = {
        "http_api" => "job_posting_api_record"
      }.freeze
      ...
      def call
        ...
        selected_vacancies = search ? vacancies.select { matches_search?(_1) } : vacancies

        observations = selected_vacancies.map do |vacancy|
          RecordSourceObservation.call(
            ...
            source_published_at: vacancy.published_at,
      ...
      def selected_strategy
        @selected_strategy ||= begin
          unless SourceRegistry.enabled?(SOURCE_KEY)
            raise UnsupportedSourceStrategy, "Remote OK acquisition source is disabled"
          end
          ...
          unless selected
            raise UnsupportedSourceStrategy, "Remote OK acquisition strategy #{requested_strategy.inspect} is not configured"
          end
          if selected.fetch("status") == "disabled"
            raise UnsupportedSourceStrategy, "Remote OK acquisition strategy #{selected.fetch('type').inspect} is disabled"
          end
      ...
      def persisted_transport
        TRANSPORT_MAP.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "unsupported Remote OK acquisition strategy #{strategy_type.inspect}"
        end
      end

      def adapter_version
        ADAPTER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Remote OK adapter version for #{strategy_type.inspect}"
        end
      end

      def parser_version
        PARSER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Remote OK parser version for #{strategy_type.inspect}"
        end
      end

      def parser
        @parser ||= parser_override || ApiParser.new
      end
      ...
      def default_run_key
        identity = [ request_url, search ].compact.join("\0")
        "remoteok:#{strategy_type}:#{Digest::SHA256.hexdigest(identity)[0, 16]}:#{started_at.iso8601(6)}"
      end

      def matches_search?(vacancy)
      ...
      def raise_previous_failure(source_run)
        ...
        raise RunFailed, "Remote OK source run already failed#{": #{message}" if message.present?}"
      end
