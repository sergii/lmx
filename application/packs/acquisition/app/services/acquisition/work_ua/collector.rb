    ...
    class Collector
      ...
      def selected_strategy
        @selected_strategy ||= begin
          unless SourceRegistry.enabled?(SOURCE_KEY)
            raise UnsupportedSourceStrategy, "Work.ua acquisition source is disabled"
          end
          ...
          unless selected
            raise UnsupportedSourceStrategy, "Work.ua acquisition strategy #{requested_strategy.inspect} is not configured"
          end
          if selected.fetch("status") == "disabled"
            raise UnsupportedSourceStrategy, "Work.ua acquisition strategy #{selected.fetch('type').inspect} is disabled"
          end
      ...
      def persisted_transport
        TRANSPORT_MAP.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "unsupported Work.ua acquisition strategy #{strategy_type.inspect}"
        end
      end

      def adapter_version
        ADAPTER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Work.ua adapter version for #{strategy_type.inspect}"
        end
      end

      def parser_version
        PARSER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Work.ua parser version for #{strategy_type.inspect}"
        end
      end

      def parser
        @parser ||= parser_override || ListingParser.new
      end
      ...
      def request_url
        @request_url ||= begin
          path = search ? "jobs-#{URI.encode_www_form_component(search)}/" : "jobs/"
          URI.join(source_base_url, path).to_s
        end
      end

      def default_run_key
        "work_ua:#{strategy_type}:#{Digest::SHA256.hexdigest(request_url)[0, 16]}:#{started_at.iso8601(6)}"
      end
      ...
      def raise_previous_failure(source_run)
        ...
        raise RunFailed, "Work.ua source run already failed#{": #{message}" if message.present?}"
      end
