    ...
    class Collector
      ...
      def selected_strategy
        @selected_strategy ||= begin
          unless SourceRegistry.enabled?(SOURCE_KEY)
            raise UnsupportedSourceStrategy, "Robota.ua acquisition source is disabled"
          end
          ...
          unless selected
            raise UnsupportedSourceStrategy, "Robota.ua acquisition strategy #{requested_strategy.inspect} is not configured"
          end
          if selected.fetch("status") == "disabled"
            raise UnsupportedSourceStrategy, "Robota.ua acquisition strategy #{selected.fetch('type').inspect} is disabled"
          end
      ...
      def persisted_transport
        TRANSPORT_MAP.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "unsupported Robota.ua acquisition strategy #{strategy_type.inspect}"
        end
      end

      def adapter_version
        ADAPTER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Robota.ua adapter version for #{strategy_type.inspect}"
        end
      end

      def parser_version
        PARSER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Robota.ua parser version for #{strategy_type.inspect}"
        end
      end
      ...
      def request_url
        @request_url ||= begin
          uri = URI.join(API_BASE_URL, "vacancy/search")
          params = {
            sortBy: "Date",
            count: PAGE_SIZE
          }
          params[:keyWords] = search if search
      ...
      def default_run_key
        "robota_ua:#{strategy_type}:#{Digest::SHA256.hexdigest(request_url)[0, 16]}:#{started_at.iso8601(6)}"
      end
      ...
      def raise_previous_failure(source_run)
        ...
        raise RunFailed, "Robota.ua source run already failed#{": #{message}" if message.present?}"
      end
