    ...
    class Collector
      ...
      def selected_strategy
        @selected_strategy ||= begin
          unless SourceRegistry.enabled?(SOURCE_KEY)
            raise UnsupportedSourceStrategy, "Djinni acquisition source is disabled"
          end
          ...
          unless selected
            raise UnsupportedSourceStrategy, "Djinni acquisition strategy #{requested_strategy.inspect} is not configured"
          end
          if selected.fetch("status") == "disabled"
            raise UnsupportedSourceStrategy, "Djinni acquisition strategy #{selected.fetch('type').inspect} is disabled"
          end
      ...
      def persisted_transport
        TRANSPORT_MAP.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "unsupported Djinni acquisition strategy #{strategy_type.inspect}"
        end
      end

      def adapter_version
        ADAPTER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Djinni adapter version for #{strategy_type.inspect}"
        end
      end

      def parser_version
        PARSER_VERSIONS.fetch(strategy_type) do
          raise UnsupportedSourceStrategy, "missing Djinni parser version for #{strategy_type.inspect}"
        end
      end

      def parser
        @parser ||= parser_override || FeedParser.new
      end
      ...
      def raise_previous_failure(source_run)
        ...
        raise RunFailed, "Djinni source run already failed#{": #{message}" if message.present?}"
      end
