...
module Integration
  module Read
    module Adapters
      class CandidateProfileGet < Ports::Query
        CONTRACT_IDENTIFIER = "candidates.profile.v1"
        PROVENANCE = { adapter: "talent_profile.public_api" }.freeze

        def initialize(candidate_api:, workspace_scope:, not_found_errors: [])
          unless candidate_api.respond_to?(:fetch_latest_profile)
            raise Error::InvalidInput.new("candidate_api must respond to fetch_latest_profile")
          end
        ...
        def call(query)
          unless query.contract.identifier == CONTRACT_IDENTIFIER
            raise Error::Unsupported.new(
              "Candidate profile query adapter does not support this contract",
